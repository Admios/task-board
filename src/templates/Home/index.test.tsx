...
it('should open the AddColumnModal when button is pressed', async () => {
  ...
  // Click the button
  const button = screen.getAllByRole('button').find((button) => button.textContent === 'Add Column')
  if (!button) {
...
it('should launch logout when button is pressed', async () => {
  ...
  // Click the button
  const button = screen.getAllByRole('button').find((button) => button.textContent === 'Logout')
  if (!button) {
    throw new Error('Button not found')
  }
  expect(button).toBeInTheDocument();
